...
export default {
  setup() {
    ...
    // 在setup函数中添加模拟控制开关
const mockMode = ref(true); // 测试时设为true
const shouldFail = ref(Math.random() > 0.5); // 50%失败率

// 测试数据生成器
const generateMockData = () => ({
  news: Array.from({length: 3}, (_,i) => ({
    newsId: i+1,
    news: `#${['量子计算','珞珈论坛','樱花季'][i]}# 最新动态 ${i+1}`,
    popularity: Math.floor(Math.random()*5000 + 5000)
  })),
  events: Array.from({length: 2}, (_,i) => ({
    eventId: `E${i+1}`,
    event: `${['学术峰会','校友返校日'][i]}即将举办`,
    popularity: Math.floor(Math.random()*3000 + 2000)
  })),
  keywords: Array.from({length: 4}, (_,i) => ({
    keyword: ['人工智能', '碳中和', '#校园安全#', '交叉学科论坛'][i],
    frequency: Math.floor(Math.random()*10000 + 5000)
  }))
});
    ...
    if(shouldFail.value) {
      ...
      keywords.value = mockData.keywords;
      events.value = mockData.events;
      //news.value = mockData.news;
    }
    loading.value = false;
    return;
  }
